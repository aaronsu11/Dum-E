...
`policy_guard/groot_guard.py` is shared by preflight and post-load validation. Refusal clears policy and both processors; an aborted setup must not leave a usable rejected model. Check the actual decoder, embodiment statistics and singular checkpoint `use_relative_action`; the generic config's identity `normalization_mapping` is normal for this processor and is not by itself a failure.

The native implementation is pinned to `23ace64f17aa5015259b8609d371eb61a357c776`; LeRobot to 0.6.1. Cosmos `9ce19a195e423419c349abfc86fd07178b230561` is a forward reproducibility pin, not recovered training provenance. The cache pins `refs/main` because upstream processor loading cannot accept a revision. `policy_guard/native_cache.py` preserves native offline cache resolution without modifying checkpoint blobs.
