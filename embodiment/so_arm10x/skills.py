...
Design notes (Phase 03, D-07/D-08/D-09):
...
class PickSkill(Skill):
    ...
    The body is moved VERBATIM from the former ``agent.pick()`` helper (Phase 03
    D-07): it drives N1.7 inference and streams the resulting action chunk to the
    arm. The ``action_horizon`` default is set to 16 to match the trained/eval
    checkpoint (RESEARCH §Pitfall 2 — the legacy default of 8 truncated the
    learned chunk); it remains configurable.
    """
