...
from embodiment.so_arm10x.validation import DEFAULT_PROTOCOL, bounded_command, check_camera, joint_limits, probe_target
...
SOURCES = ("embodiment/so_arm10x/schema.py", "policy/backends/galaxea/protocol.py","policy/factory.py", "policy/configuration.py", "policy/execution/rtc.py",
           "embodiment/so_arm10x/mappings/lerobot.py", "shared/interfaces.py",
           "policy/backends/lerobot/models/molmoact2.py", "policy/backends/lerobot/models/groot.py",
           "scripts/run_policy_trial.py", "embodiment/so_arm10x/trial.py", "embodiment/so_arm10x/validation.py",
           "policy/backends/galaxea/backend.py", "embodiment/so_arm10x/mappings/galaxea.py",
            "policy/backends/lerobot/http_client.py", "embodiment/so_arm10x/mappings/frames.py",
           "policy/evidence.py", "embodiment/so_arm10x/observation.py",
           "policy/backends/lerobot/pi05_client.py", "policy/backends/lerobot/models/pi05.py",
           "embodiment/so_arm10x/rtc.py", "policy/backends/lerobot/runtime.py", "policy/backends/lerobot/http_server.py",
           "policy/backends/lerobot/models/pi05_so101_manifest.json",
           "policy/checkpoints.py", "policy/backends/lerobot/transports/http.py",
...
def run(workspace):
    ...
    profile = "g05-so101" if PROTOCOL.get("motion_probe") else PROTOCOL["profile"]
    deployment = deployment_for_profile(profile, execution=PROTOCOL.get("scheduler", "sync"))
    kwargs = {"port": PROTOCOL["policy_port"], "language_instruction": PROTOCOL["instruction"]}
    if profile == "groot-so101":
        kwargs["calibration_path"] = snapshot["calibration_path"]
...
def main():
    ...
    parser.add_argument("--profile", choices=["g05-so101", "molmoact2-so101", "pi05-so101", "groot-so101"],
                        default=None)
    parser.add_argument("--scheduler", choices=["sync", "rtc"], default=None)
    ...
    if args.deployment:
        ...
        if args.profile == "lerobot-gr00t":
            parser.error("This bounded trial command uses HTTP or Galaxea; use the GR00T async agent workflow")
        if args.scheduler is not None and args.scheduler != deployment.execution:
            parser.error("--scheduler conflicts with deployment execution")
        args.scheduler = deployment.execution
    args.profile = args.profile or "g05-so101"
    args.scheduler = args.scheduler or "sync"
