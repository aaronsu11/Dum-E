...
class GalaxeaPolicyBackend(IPolicyBackend):
    def __init__(self, host="127.0.0.1", port=None, camera_keys=None,
        ...
        if robot_state_keys is not None and set(robot_state_keys) != set(mapping.JOINTS):
    ...
    def get_action(self, observation_dict, lang=None, *, seed=None):
        with self._lock:
            self._session.reset()
            raw = self.mapping.make_observation(observation_dict, lang or self._instruction, seed=seed)
            actions = []
            try:
                for index in range(32):
                    ...
                    actions.append(self.mapping.decode_action(reply["action"]))
                    if index == 0:
