...
* ``_init_socket`` closes the previous REQ socket with ``LINGER=0`` before
  ...
  ``connect`` so a recv against a dead server cannot block forever.
* ``call_endpoint`` retries a bounded number of times (``max_retries``,
  ...
  raises a descriptive ``RuntimeError`` naming ``host:port``. Worst-case
  total wait is ``timeout_ms * max_retries`` (~45s with the 15s default),
  comfortably inside the agent's long-running ``function_call_timeout_secs``.
...
class MsgSerializer:
    ...
    def decode_custom_classes(obj):
        ...
        if "__ndarray_class__" in obj:
            ...
            # execute code. Matches upstream server_client.py (security fix).
    ...
    def call_endpoint(
        ...
        for _attempt in range(max_retries):
            try:
                ...
                # the next attempt (strict-REQ teardown).
                last_err = exc
                self._init_socket()
                continue
...
class ExternalRobotInferenceClient(BaseInferenceClient):
    ...
    chunk before it reaches the robot action loop.
    """
    ...
    def _validate_action_chunk(action_chunk: Any, modality_keys) -> None:
        ...
        bare ``KeyError``/``IndexError`` deep in the action loop.
        """
