"""Loopback HTTP policy client; the embodiment supplies its coordinate mapping."""
import json
import os
import threading
...
from shared import IPolicyBackend, IPolicyMapping


class HTTPPolicyBackend(IPolicyBackend):
    def __init__(self, port=18081, language_instruction=None, *, profile,
                 mapping: IPolicyMapping, host="127.0.0.1"):
        ...
        if host not in ("127.0.0.1", "localhost"):
            raise ValueError("HTTP inference requires a loopback server or tunnel")
        ...
        if (len(mapping.joint_names) != self.profile.action_dim
                or len(set(mapping.joint_names)) != len(mapping.joint_names)
                or set(mapping.camera_names) != {"front", "wrist"}):
            raise ValueError("Mapping does not match the checkpoint action dimension or HTTP camera schema")
    ...
    def _to_model(self, values):
        return self.mapping.to_model(values)

    def _to_arm(self, values):
        return self.mapping.to_arm(values)
    ...
    def get_action(self, observation_dict, lang=None, *, seed=20265907):
        ...
        state = np.asarray(self._to_model([observation_dict[k] for k in self.mapping.joint_names]))
        if (state.shape != (self.profile.action_dim,) or state.dtype.kind not in "fiu"
                or not np.isfinite(state).all()):
            raise ValueError("Mapping produced invalid model state")
        images = {}
        for role in self.mapping.camera_names:
        ...
        with self._lock:
            ...
            if (values.shape != (self.profile.horizon, len(self.mapping.joint_names)) or values.dtype.kind not in "fiu"
            ...
            mapped = np.asarray([self._to_arm(v) for v in values])
            if (mapped.shape != values.shape or mapped.dtype.kind not in "fiu"
                    or not np.isfinite(mapped).all()):
                raise ValueError("Mapping produced invalid arm targets")
            actions = [dict(zip(self.mapping.joint_names, map(float, v))) for v in mapped]
            self.mapping.validate()
            self.last_metadata.update(self.mapping.metadata)
            return actions
