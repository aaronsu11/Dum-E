...
def assert_groot_serving_contract(snapshot: GrootGuardSnapshot) -> None:
    ...
    if snapshot.served_letter_box_transform != SERVING_LETTER_BOX_TRANSFORM:
        raise ValueError(
            ...
            f"policy/backends/lerobot/server.py did not land — most likely the step's registry "
...
def load(runtime):
    ...
    from policy.backends.lerobot import server
    config = GrootConfig(base_model_path=runtime.snapshot, embodiment_tag=EXPECTED_TAG,
                         model_params_fp32=False)
    server.fixup_policy_features(config, ["front", "wrist"], 480, 640, 6, 6)
    runtime.policy = server.DumEGrootPolicy.from_pretrained(runtime.snapshot, config=config)
    runtime.pre, runtime.post = make_pre_post_processors(
        runtime.policy.config, pretrained_path=runtime.snapshot,
    ...
    assert_groot_serving_contract(
        snapshot_from_loaded(runtime.policy.config, runtime.pre, runtime.post, runtime.profile.horizon)
    )
