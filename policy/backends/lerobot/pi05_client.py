...
from policy.backends.lerobot.transports.http import encode_image, prefix_digest


class Pi05SO101PolicyBackend(HTTPPolicyBackend):
    def __init__(self, host="127.0.0.1", port=None, camera_keys=None,
                 robot_state_keys=None, show_images=False, language_instruction=None, *, mapping):
        ...
        if robot_state_keys is not None and set(robot_state_keys) != set(mapping.joint_names):
        ...
        super().__init__(port=port or int(os.getenv("DUME_PI05_POLICY_PORT", "18081")),
                         language_instruction=language_instruction, profile="pi05-so101", mapping=mapping)
    ...
    def get_rtc_action(self, observation, prefix, *, epoch, request_id, delay_steps):
        ...
        state = np.asarray([observation[k] for k in self.mapping.joint_names], dtype=float)
