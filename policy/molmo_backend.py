"""Experimental synchronous SO101 bridge to the pinned Molmo evaluation server.

The server does not certify physical readiness. The attended integration runner
owns calibration checks, live camera validation, bounded dispatch and safe hold.
"""
import json
import os
import threading
...
class MolmoPolicyBackend(IPolicyBackend):
    def __init__(self, port=18081, language_instruction=None):
        if os.getenv("DUME_ASYNC_INFERENCE", "0") == "1":
            raise ValueError("Molmo controller bridge supports synchronous chunks only")
        ...
        self.profile = get_profile("molmoact2-so101")
    ...
    def _check_health(self, health):
        if (health.get("status") != "ready" or health.get("fault")
            ...
            raise ValueError("Molmo server not ready or pinned checkpoint/profile mismatch")
