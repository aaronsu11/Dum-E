...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Simulated control/failure through the active agent/MCP/voice stack; no hardware")
    p.add_argument("mode", choices=("failure", "control"))
    p.add_argument("--workspace", type=Path, required=True)
    args = p.parse_args()
    args.workspace.mkdir(parents=True, exist_ok=False)
    asyncio.run((failure if args.mode == "failure" else control)(args.workspace))
