"""One approved voice/MCP-triggered physical pick through the guarded trial runner.

The voice model selects the task. Physical execution uses the fixed approved
banana instruction, 320-action budget and original stop/clamp composition.
"""
import argparse
import asyncio
...
from scripts import run_async_physical_trial as physical
...
physical.PROTOCOL={**physical.PROTOCOL,'trigger':'one voice/MCP banana pick','retarget_allowed':False}
physical.SOURCES=(*physical.SOURCES,'scripts/run_voice_physical_trial.py','mcp_server.py','pipecat_server.py')
    ...
    async def control_loop():
        ...
        async for message in broker.subscribe(message_types=[MessageType.STATUS_UPDATE],task_id=task_id):
            ...
            if data.get('status')=='cancelled':
                ...
                await publish(MessageType.TASK_PROGRESS,{'message':'Retargeting is not enabled for this single approved physical trial.'})
    print('READY: Physical Test Arm, exact ID physical-test-arm; waiting for one spoken banana pick.',flush=True)
    try:
...
if __name__=='__main__':
    ...
    elif args.command=='execute':raise SystemExit(physical.run(args.workspace))
