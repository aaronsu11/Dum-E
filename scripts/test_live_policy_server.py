...
wire contract that the mocked unit tests can only simulate:
  ...
  3. unreachable-raises       — stop the server, confirm a descriptive RuntimeError
                                within ~45s instead of an infinite hang (GR-03/D-06)
  4. (optional) wrong tag     — schema/embodiment mismatch fails loudly (GR-02/D-04)

This covers GR-01/GR-03/GR-04 against real bytes. It does NOT cover the physical
arm-motion portion of GR-07 (that still needs the SO101 hardware gate).
...
# Import the project client wrapper (verified in Phase 3).
...
def test_unreachable(host: str, port: int) -> bool:
    ...
    print("       (validates GR-03/D-06: bounded retry -> descriptive RuntimeError, no hang)")
    try:
