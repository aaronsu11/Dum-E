...
def test_native_duck_typed_batch_feature():
    from policy.telemetry import tensor_metadata
    class Batch:
...
def test_lerobot_entrypoint_routes_prediction_through_shared_observer(monkeypatch, mode):
    ...
    def serve():
        ...
        return 0
    entry.main=serve
    monkeypatch.setitem(sys.modules, 'policy.backends.lerobot.preflight', entry)
...
def test_native_endpoint_uses_shared_observer(monkeypatch, tmp_path, mode):
    ...
    from policy.backends.isaac_groot import cache as native_cache
