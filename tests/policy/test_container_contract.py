...
def _load_container_entrypoint():
    ...
    path = REPO_ROOT / "policy" / "backends" / "lerobot" / "preflight.py"
...
def _load_container_server():
    ...
    path = REPO_ROOT / "policy" / "backends" / "lerobot" / "server.py"
...
def _run_build_wrapper_sandboxed(tmp_path, dotenv_body: str):
    ...
    (root / "docker" / "lerobot").mkdir(parents=True)
    shutil.copy(BUILD_WRAPPER, root / "scripts" / BUILD_WRAPPER.name)
    (root / "docker" / "lerobot" / "Dockerfile").write_text("FROM scratch\n")
...
def test_preflight_numbered_lines_track_total_checks():
    ...
    entrypoint = _load_container_entrypoint()
    source = (REPO_ROOT / "policy" / "backends" / "lerobot" / "preflight.py").read_text(
        encoding="utf-8"
    )
