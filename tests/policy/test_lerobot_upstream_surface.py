...
#: does not carry (``tests/policy/test_container_contract.py``'s dependency-isolation
#: guard is what keeps it out).
SERVER_PY = Path(__file__).resolve().parents[2] / "policy" / "backends" / "lerobot" / "server.py"
...
REAL_CHECKPOINT = Path(__file__).resolve().parents[2] / "tests" / "fixtures" / "groot-so101"
...
def test_dume_copy_assigns_exactly_upstream_send_policy_instructions_state():
    ...
    extra = ours - upstream
    assert not missing, (
        f"policy/backends/lerobot/server.py's SendPolicyInstructions copy does not assign "
...
def test_remote_policy_config_fields_the_copy_reads_still_exist():
    ...
    assert not missing, (
        ...
        f"SendPolicyInstructions copy and policy/backends/lerobot/session.py's connect() build this "
...
def test_dume_copy_reads_exactly_upstream_create_groot_model_config_fields():
    ...
    extra = ours - upstream
    assert not missing, (
        f"policy/backends/lerobot/server.py's _create_groot_model copy does not read "
...
def test_tie_unused_qwen_lm_head_still_exists():
    ...
    assert callable(_tie_unused_qwen_lm_head), (
        ...
        "It is a PRIVATE symbol imported deliberately by policy/backends/lerobot/server.py "
...
def test_model_params_fp32_defaults_true_and_is_what_casts_weights_up():
    ...
    assert GrootConfig.__dataclass_fields__["model_params_fp32"].default is True, (
        ...
        "in policy/backends/lerobot/server.py before assuming the injection is still needed."
    )
...
#: Every ``PolicyServer`` member ``policy/backends/lerobot/server.py`` overrides or
...
#: contract between ``policy/backends/lerobot/session.py``, ``DumEGrootPolicyServer`` and
...
def test_policy_server_still_exposes_the_overridden_methods():
    ...
    assert not missing, (
        ...
        f"policy/backends/lerobot/server.py overrides or calls every one of "
...
def test_remote_policy_config_field_names_are_unchanged():
    ...
    assert set(RemotePolicyConfig.__dataclass_fields__) == {
        ...
        f"{sorted(RemotePolicyConfig.__dataclass_fields__)}. policy/backends/lerobot/session.py builds "
        f"this payload and policy/backends/lerobot/server.py unpacks it; re-diff both against "
...
def test_timed_observation_carries_must_go():
    ...
    assert "must_go" in TimedObservation.__dataclass_fields__, (
        ...
        f"{sorted(TimedObservation.__dataclass_fields__)}). policy/backends/lerobot/session.py passes "
    ...
    assert callable(getattr(TimedAction, "get_action", None)), (
        "TimedAction.get_action is gone. policy/backends/lerobot/backend.py reads every decoded "
...
def test_async_inference_service_has_the_four_expected_methods():
    ...
    assert set(service.methods_by_name) == set(ASYNC_INFERENCE_METHODS), (
        ...
        f"policy/backends/lerobot/session.py, policy/backends/lerobot/server.py and "
    ...
    assert not missing, (
        ...
        f"policy/backends/lerobot/session.py's calls would raise AttributeError at connect time."
    )
    ...
    assert callable(services_pb2_grpc.add_AsyncInferenceServicer_to_server), (
        ...
        "policy/backends/lerobot/preflight.py and scripts/mock_grpc_policy_server.py both "
...
def test_groot_horizon_and_tag_helpers_still_resolve_sixteen():
    ...
    from policy.backends.lerobot.models.groot import EXPECTED_HORIZON, EXPECTED_TAG
...
def test_checkpoint_processor_assets_loader_is_the_pinned_private_surface():
    "``policy/backends/lerobot/models/groot.py`` CALLS this private symbol; pinned at 06-02's request."
    ...
    assert callable(_load_n1_7_checkpoint_processor_assets), (
        ...
        "gone or not callable. policy/backends/lerobot/models/groot.py derives BOTH assets_present and "
    ...
    assert list(parameters) == ["config"], (
        ...
        f"['config']. policy/backends/lerobot/models/groot.py calls it with a single GrootConfig."
    )
