...
- ``allow_pickle=False`` on the decode path (security)
...
def test_decode_uses_allow_pickle_false():
    ...
    object_hook and is not cleanly mockable. This guards against a malicious
    pickle in ``as_npy`` being able to execute code.
    """
